...
| `src/i18n.js`              | 中英文翻译字符串（`MENU_I18N`）；翻译查询函数 `t()`；语言切换（`switchLanguage`）；界面语言刷新（`applyLanguageToUI`） | 整个软件的多语言支持                                         |
...
| `src/menu.js`              | 菜单栏动态构建（`buildMenuBar`、`buildMenuItems`）；About 对话框（`menuAbout`、`closeAbout`） | 菜单栏 UI、About对话框                                       |
...
- debug: 步数超千万时打开渲染面板直接内存溢出
- debug: 直接修改渲染设置里的输出路径文本框而不是是用文件选择器，这种修改不会被视为修改文件，在保存时也会被忽略
- 有倍速时优化声音的连贯性
- 渲染动画中纸带分行
- 设置动画渲染中有向图到画面边缘的距离
- 探索直接编码成视频文件的可行性
